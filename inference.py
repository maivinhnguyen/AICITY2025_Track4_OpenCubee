...
def process_detections(model, test_folder, output_file, conf_thresholds, batch_size=16):
    """
    ...
    for i in tqdm(range(0, len(image_files), batch_size), desc="Processing batches"):
        ...
        batch_paths = [os.path.join(test_folder, img_file) for img_file in batch_files]
        
        # Run inference on batch
        batch_results = model(batch_paths)
        
        # Process each result
        for img_file, predictions in zip(batch_files, batch_results):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", type=str, default="", 
                        help="Device to run inference on (e.g., '0,1' for GPUs 0 and 1, or 'cpu')")
    ...
    # Process detections
    process_detections(model, args.test_folder, args.output, conf_thresholds, args.batch_size)